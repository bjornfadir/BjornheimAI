...
| Human and runtime bootstrap configuration | Protected `users.yaml` remains the Telegram-human bootstrap input; protected `runtime-profiles.yaml` is the installed runtime-profile identity/selection authority | Telegram user ID for human bootstrap; opaque runtime profile ID for execution policy | Existing runtime profiles retain private integer compatibility storage keys until pool and state namespaces finish migrating. |
...
| `users.yaml`, `runtime-profiles.yaml`, `workspaces.yaml`, `memory-projects.yaml` | Operator-managed installation policy/bootstrap | **Retain.** `runtime-profiles.yaml` is independently loaded protected execution policy; `users.yaml` remains Telegram-human bootstrap input during migration. Workshop state may become mutable domain data, but protected files remain the safe bootstrap and installation-policy surface. |
...
2. **Canonical timeline query contract:** implemented as a transport-independent, paginated channel timeline reader with stable cursors and mandatory principal/channel authorization. The live HTTP adapter accepts only canonical channel IDs and bearer sessions; it replaces no JSONL reads.
...
The client-state isolation required by [#835](https://github.com/dcellison/kai/issues/835) separates mutable device, session, grant, and revocation rows from projection-owned principal deletion. Schema migration preserves existing security rows and explicit replay/restart tests prove that active and revoked state survives canonical projection rebuilds. Registration includes bounded enrollment attempts and an explicit loopback/TLS-termination assumption. Installed enrollment, restart, and revocation evidence has passed.

Successful installed voice ingress, service restart, and clean canonical-to-JSONL parity are recorded, including a clean post-outbox install at 32 canonical/projected messages. Binding-aware terminal facts and per-binding FIFO claims are implemented without startup registration or production enqueue. The deliberately invoked installed qualification path is available to collect real Telegram delivery and restart-recovery evidence without making the outbox the production authority. Operator-facing client grant issuance and read-only routes are live. Installed client enrollment, timeline access, restart persistence, revocation, and denial-after-revocation passed on 2026-08-12 using a disposable device; the credential was then revoked and removed. A second disposable device qualified timeline-to-stream handoff, restart resume, later live delivery, and denial after revocation on 2026-08-13.
...
## 35. Authenticated resumable timeline event stream

**Implementation date:** 2026-08-12

The live read-only Workshop client surface now includes a Server-Sent Events
endpoint for authorized canonical channel messages. A fresh connection can
start at the current channel boundary or accept an explicit initial event
position after a completed timeline snapshot. Reconnecting clients resume from
the standard `Last-Event-ID` header, which takes precedence over the initial
query position. Each versioned event contains one sanitized canonical message
and uses that message's durable event position as its SSE identity. Raw event
envelopes, backend output, credentials, transport identifiers, and events from
other channels never enter the stream.

The stream authenticates and authorizes before parsing untrusted routing or
resume input. It rechecks channel membership during every bounded poll and
revalidates the durable bearer session periodically, so revocation closes an
existing stream without turning every idle poll into a SQLite write. Database
operations briefly acquire the client API's shared lock; network waits and
backpressure never retain it. Concurrent streams are bounded per principal and
across the process, and capacity is released on disconnect or shutdown.
Positions ahead of the durable event log return a bounded resynchronization
requirement rather than silently skipping future messages. Heartbeats carry no
event identity and therefore cannot advance a client checkpoint.

Automated coverage proves ordered backlog replay, a future-only live update,
restart and `Last-Event-ID` recovery, header precedence, ongoing session
revocation, cross-channel and unknown-channel denial equivalence, bounded
invalid and ahead-of-log positions, idle authentication-write throttling,
concurrent-stream limits, security headers, read-only routing, and production
registration. The endpoint does not accept commands, start a run, invoke a
backend, alter Telegram delivery, or replace any JSONL reader.

### 35.1 Installed qualification
...
## 36. Preliminary read-only Workshop browser shell

**Implementation date:** 2026-08-13

The first browser-facing Workshop slice is a deliberately preliminary static
reference client served at `/workshop/`. It redeems an operator-issued grant,
keeps the resulting bearer credential in tab-scoped `sessionStorage`, reads the
complete snapshot-stable canonical timeline, and continues through an
authenticated `fetch`-based event stream using the timeline boundary and
`Last-Event-ID`. It safely renders message content through text nodes, dedupes
message IDs across snapshot and stream, reconnects after interruption,
resynchronizes after a rejected future boundary, and forgets expired or revoked
sessions. A valid redeemed session survives channel-ID correction within the
same tab instead of forcing another enrollment.

The shell is read-only. It registers no command, message, run, backend, or
delivery write route. Its packaged HTML, CSS, and JavaScript are no-store,
same-origin resources with a restrictive content security policy, framing and
referrer denial, cross-origin isolation headers, and no inline code. A plain
...
The mixed loopback HTTP server and dedicated Workshop LAN listener now have an
explicit `HttpAdapter` lifecycle under the same host. It owns listener startup,
readiness, supervision, partial-start cleanup, and shutdown. Because adapters
stop in reverse attachment order, HTTP ingress closes before the Telegram
adapter and core services. Health now reports both HTTP listener components and
Telegram components through the common adapter-readiness contract.
...
and ordinary authenticated GitHub webhook notifications targeting a configured
Telegram notification group now enter that canonical feed. One database
transaction records the notification message and its durable Telegram outbox
request. GitHub delivery identity plus destination binding supplies stable
idempotency, so repeated delivery or multiple subscribed humans cannot create
duplicate feed entries or Telegram sends. The existing direct-send route
remains only as compatibility behavior for destinations that are not yet bound
to a canonical Workshop notification channel.

The browser receives each entry through its live channel stream, while the
dedicated notification-purpose Telegram worker delivers the same entry to the
existing group. Specialized issue triage remains unchanged until it receives
