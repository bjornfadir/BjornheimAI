...
class ClaudeCodeBackend(AgentBackend):
    ...
    async def _ensure_started(self) -> None:
        ...
        claude_cmd = [
            ...
            # per turn. Passed as a CLI flag (verified against `claude
            # --help`) rather than via a --settings JSON blob because CLI
            # flags fail loudly on typo at subprocess startup. Future
            # maintainer: do NOT migrate this into --settings JSON without
            # first re-verifying the key name against `claude --help`. The
            # value is validated at config load (config.py
            # _VALID_EFFORT_LEVELS) so by the time it reaches this point it
            # is guaranteed to be one of the five accepted strings.
            "--effort",
        ...
        if effective_claude_user:
            ...
            #
            # TMPDIR is preserved so the per-os-user TMPDIR anchoring
        ...
        # content-hashes any CLI-provided settings JSON into
        ...
        # so every claude temp write is namespaced per user and a
        # future --settings use cannot reintroduce the collision.
        # Single-user mode (no claude_user) uses the system default
    ...
    async def _async_sudo_kill(self, target_user: str, pid: int, sig: int) -> None:
        """
        Signal the inner claude PID through the canonical cross-user
        helper in kai.acp.
        ...
        already-exited race (inner claude died between the PID cache
        ...
        if self._pgid is not None:
            ...
            if self._effective_claude_user is not None:
                ...
                if self._inner_claude_pid is not None:
                    ...
                    # and swallowed inside the helper, so control
                    # always falls through to the killpg below -
                    # the wrapper still needs reaping regardless of
                    # how the inner-PID signal went.
                    _signal_target_user_pid_sync(
                        target_user=self._effective_claude_user,
                        pid=self._inner_claude_pid,
                        sig=int(sig),
                        purpose="chat",
                        backend=self.backend_name,
                    )
